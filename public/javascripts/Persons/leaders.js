$(function () {
    ...
    $.ajax({
        ...
        $("#jsGrid").jsGrid({
            ...
            deleteConfirm: "Do you really want to delete Leader ?",
            headerRowClass: 'table-green-header',

            fields: [
                {type: "control"},

                {name: "selectedLeader", title: "Select", type: "_checkboxDateField", width: 40, align: "center"},
                {
                    name: "Full_Names",
                    title: "Full Names",
                    type: "text",
                    ...
                    autosearch: true,
                    filtering: true,
                    message: "Please enter Full Names",
                    validate: "required"
                },
                ...
                {
                    name: "leaderCounty", title: "County", type: "text", width: 100, autosearch: true, required: false,

                },

                {
                    name: "leaderConstituency",
                    title: "Constituency",
                    type: "text",
                    width: 100,
                    autosearch: true,
                ...
                {
                    name: "leaderWard", title: "Ward", type: "text", width: 100, autosearch: true,
                    //  validate: "required"
                },
...
*/
                ...
                {
                    name: "leaderCreatedBy", title: "CreatedBy", editing: false, readOnly: "true",
                    width: 200
                },
                {name: "leaderDateCreated", title: "DateCreated", editing: false, readOnly: "true", width: 200},
            ...
            controller: {
                loadData: function (filter) {
                    return $.grep(data, function (item) {
                        ...
                        return (!filter.selectedLeader || item.selectedLeader.indexOf(filter.selectedLeader) > -1)
                            && (!filter.Full_Names || item.Full_Names.toLowerCase().indexOf(filter.Full_Names.toLowerCase()) > -1)
                            && (!filter.Status || item.Status.toLowerCase().indexOf(filter.Status.toLowerCase()) > -1)
                            && (!filter.PositionType || item.PositionType.toLowerCase().indexOf(filter.PositionType.toLowerCase()) > -1)
                            && (!filter.leaderCounty || item.leaderCounty.toLowerCase().indexOf(filter.leaderCounty.toLowerCase()) > -1)
                            && (!filter.leaderConstituency || item.leaderConstituency.toLowerCase().indexOf(filter.leaderConstituency.toLowerCase()) > -1)
                            && (!filter.leaderWard || item.leaderWard.toLowerCase().indexOf(filter.leaderWard.toLowerCase()) > -1)
                            && (!filter.leaderComments || item.leaderComments.toLowerCase().indexOf(filter.leaderComments.toLowerCase()) > -1)
                            && (!filter.leaderCreatedBy || item.leaderCreatedBy.toLowerCase().indexOf(filter.leaderCreatedBy.toLowerCase()) > -1)
                            && (!filter.leaderDateCreated || item.leaderDateCreated.indexOf(filter.leaderDateCreated) > -1)
